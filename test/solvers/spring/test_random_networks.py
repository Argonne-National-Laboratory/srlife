...
def copy_network(base, tube_length, tube_ri, E, alpha, tforce):
  ...
  for a, b, data in base.edges(data = True):
    if a[:4] == 'tube' and b[:4] == 'tube':
      other.add_edge(a,b, object = gen_tube(data['stiffness'], tforce, 
...
class TestNetworksWithDisconnects(unittest.TestCase):
  ...
  def test_random(self):
    ...
    while ran < self.ntrials:
      ...
      for p1, p2 in zip(subprobs1, subprobs2):
        disp1 = np.sort(np.array([data['displacement'] for n,data in p1.nodes(data=True)]))
  ...
  def gen_network(self):
    ...
    for i in range(self.npanels):
      pid = test.add_panel(ra.choice([self.kpc,0.8*self.kpc,1.2*self.kpc]*3+['rigid','disconnect']))
      for j in range(self.ntubes):
        test.add_tube(pid, ra.choice([self.ktc,0.8*self.ktc,1.2*self.ktc]*3+['rigid','disconnect']), self.kt)

    nn = copy_network(test, self.tube_length, self.tube_ri, self.E, self.alpha, self.tforce)
    old2new = {}
    new2old = {}
...
class TestNetworksNoDisconnects(unittest.TestCase):
  ...
  def gen_network(self):
    ...
    for i in range(self.npanels):
      pid = test.add_panel(ra.choice([self.kpc,0.8*self.kpc,1.2*self.kpc]*3+['rigid']))
      for j in range(self.ntubes):
        test.add_tube(pid, ra.choice([self.ktc,0.8*self.ktc,1.2*self.ktc]*3+['rigid']), ra.choice([self.kt,0.8*self.kt,1.2*self.kt]))
