...
class TestPiecewiseLinearThermalMaterial(CommonThermalMaterial, unittest.TestCase):
    def setUp(self):
        ...
        self.obj = materials.PiecewiseLinearThermalMaterial(
            self.name, self.T, self.cond, self.diff
        )

    def test_interpolate(self):
        T = 125.0
        self.assertTrue(
            np.isclose(inter.interp1d(self.T, self.cond)(T), self.obj.conductivity(T))
        )
        self.assertTrue(
            np.isclose(inter.interp1d(self.T, self.diff)(T), self.obj.diffusivity(T))
        )
    ...
    def test_derivtive(self):
        for T in self.Trange:
            self.assertAlmostEqual(
                self.obj.dcoefficient(self.mat, T),
...
class TestConstantFluidMaterial(CommonFluidMaterial, unittest.TestCase):
    ...
    def test_recover(self):
        ...
        self.assertAlmostEqual(
            self.obj.coefficient("Computonium", 0), comp.coefficient("Computonium", 0)
        )
...
class TestPiecewiseLinearFluidMaterial(CommonFluidMaterial, unittest.TestCase):
    def setUp(self):
        self.data = {
            "Computonium": (
                np.array([10.0, 100.0, 200.0, 300.0]),
                np.array([50, 10.0, 5.0, 2.0]),
            ),
            "Other": (
                np.array([10, 100, 200.0, 300.0]),
...
class TestStructuralMaterial(unittest.TestCase):
    def setUp(self):
        ...
        self.data = {
            "nFatigue": {
                "curve1": {
                    "T": "950",
                    "a": "-9.2e-01 -3.8e+00 -8.4e+00 -4.1e+00",
                    "n": "3 2 1 0",
                    "cutoff": "1.5e-3",
                },
                "curve2": {
                    "T": "800",
                    "a": "-19.2e-01 -3.8e+00 -18.4e+00 -14.1e+00",
                    "n": "3 2 1 0",
                    "cutoff": "1.5e-3",
                },
                "curve3": {
                    "T": "500",
                    "a": "-0.2e-01 -0.8e+00 -0.4e+00 -0.1e+00",
                    "n": "3 2 1 0",
                    "cutoff": "1.5e-3",
                },
            },
            "avgRupture": {
                "C": "17.16",
                "a": "-1475.23 7289.41 -16642.64 35684.60",
                "n": "3 2 1 0",
            },
            "lbRupture": {
                "C": "10.0",
                "a": "-1000.0 7000.0 -10000.0 30000.0",
                "n": "3 2 1 0",
            },
    ...
    def test_inside_envelope(self):
        ...
        """
        self.assertTrue(
            self.structmat.inside_envelope(
                self.cfinteraction_pname,
                self.true_inside_envelope[0],
                self.true_inside_envelope[1],
            )
        )
        self.assertFalse(
            self.structmat.inside_envelope(
                self.cfinteraction_pname,
                self.false_inside_envelope[0],
                self.false_inside_envelope[1],
            )
        )
        with self.assertRaises(ValueError):
            self.structmat.inside_envelope(
                self.cfinteraction_pname,
                self.error_inside_envelope[0],
                self.error_inside_envelope[1],
            )
