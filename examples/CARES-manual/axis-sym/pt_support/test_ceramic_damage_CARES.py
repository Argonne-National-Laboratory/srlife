...
class TestPIAModel(unittest.TestCase):
  def setUp(self):
    ...
    data1 = np.loadtxt(os.path.join(os.path.dirname(os.path.abspath(__file__)),"241.csv"),
    delimiter = ',', skiprows = 1, usecols=list(range(0,7)))
    ...
    angle = 360.0;  #segment angle
    self.vol_factor = 360.0/angle;

    self.volumes = data1[:,6]*self.vol_factor
